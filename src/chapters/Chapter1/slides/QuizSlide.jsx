...
export default function QuizSlide({ quizId, onNext, onPrev }) {
  const {
    quiz,
    currentQuestion,
    answers,
    ...
    return (
      <div className="h-full flex items-center justify-center px-4">
        <p className="text-2xl text-gray-600 text-center">
          Quiz not found.<br />
          Check quiz ID: <code>{quizId}</code>
        </p>
      </div>
    );
  }
  ...
  if (isComplete) {
    const getBadge = (percentage) => {
      if (percentage >= 90) return { emoji: '🏆', title: 'Outstanding!', color: 'from-yellow-400 to-orange-500' };
      if (percentage >= 70) return { emoji: '⭐', title: 'Excellent!', color: 'from-blue-400 to-purple-500' };
      if (percentage >= 50) return { emoji: '👍', title: 'Good Job!', color: 'from-green-400 to-blue-500' };
      return { emoji: '💪', title: 'Try Again!', color: 'from-gray-400 to-gray-600' };
    };
    ...
    return (
      <div className="h-full flex flex-col justify-center items-center px-4 py-6 overflow-y-auto">
        ...
        <h3 className="text-xl sm:text-2xl md:text-3xl text-gray-600 mb-4 md:mb-6">Quiz Completed!</h3>

        {/* Desktop: grid 2 columns | Mobile: 1 column */}
        <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 mb-6">
          ...
          <div className="bg-white rounded-2xl p-5 sm:p-6 shadow-lg w-full">
            <h4 className="text-xl sm:text-2xl font-bold text-gray-800 mb-4">Answer Review:</h4>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {!correct && userAnswer !== undefined && (
                        <p className="text-red-600 mt-1 text-sm">
                          ❌ Your answer: <span className="font-mono">{question.options[userAnswer]}</span>
                        </p>
                      )}
                      {correct && (
        ...
        {/* Navigation & Retake Buttons – FIXED: use onPrev & onNext props */}
